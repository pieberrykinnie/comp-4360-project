...
* **Model**: In the models/ module, we kept most of the original SimMIM ViT implementation, removed unused Swin branches, and adapted the architecture for chest X-rays mainly by supporting single-channel inputs and matching the SimMIM reconstruction head to the encoder input channels instead of hardcoded RGB reconstruction.
  ...
  * `optimizer.py`We wrote our own simplified optimizer module inspired by the original SimMIM optimizer, but we did not copy all of its fine-tuning machinery.
  * `utils.py` - It is derived from the original SimMIM utility design and not fully independent, but it is not line-for-line copied,it is simplified, and includes your own added logic, such as filtering out pretrained checkpoint keys that are missing from the current model or have shape mismatches.
