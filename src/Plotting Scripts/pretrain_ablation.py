...
def plot_grad_norm_vs_step(baseline, ablation, outdir, smooth_window):
    base_x = moving_average_x(baseline["global_steps"], smooth_window)
    base_y = moving_average(baseline["grad_norms"], smooth_window)

    abl_x = moving_average_x(ablation["global_steps"], smooth_window)
    abl_y = moving_average(ablation["grad_norms"], smooth_window)
    ...
    plt.title(f"Smoothed Gradient Norm vs Global Step (window={smooth_window})")
...
def main():
    ...
    plot_grad_norm_vs_step(baseline, ablation, args.outdir, args.smooth_window)
